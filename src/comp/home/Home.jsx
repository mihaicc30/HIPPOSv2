...
const Home = ({
  ...
  if (!user || user === "") {
    ...
    useEffect(() => {
      if (!user || !venueNtable.venue || !venueNtable.table)
        return nav("/");

      (async () => {
        if (!(await CheckAccess("menu"))) nav("/signout");
      })();
    }, [user]);
          ...
          >
            ...
            searchValue === "" ? (
              menuitems.map((item, index) => (
                <div
                  key={index}
                  className="p-2 flex flex-col cursor-pointer hover:scale-[0.98] transition-all border-b-2 animate-fadeUP1 opacity-0"
